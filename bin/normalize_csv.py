...
def read_gainers(filename):
    ...
    for column in gainers_df:
        ...
        if column.name in ['Price', 'Last']:
            if column.dtype == 'O':
                price = column.str.extract('(-?([0-9]*,)*[0-9]+.?[0-9]*)', expand = False).iloc[:, 0]\
        ...
        if column.name in ['Change', 'Chg']:
            if column.dtype == 'O':
                price_change = column.str.extract('(-?([0-9]*,)*[[0-9]+.?[0-9]*)', expand = False).iloc[:, 0]\
                        .str.replace(',', '').astype('float')
            else:
        ...
        if column.name in ['Change %', '% Chg']:
            if column.dtype == 'O':
                price_percent_change = column.str.extract('(-?([0-9]*,)*[[0-9]+.?[0-9]*)', expand = False).iloc[:, 0]\
                        .str.replace(',', '').astype('float')
            else:
    ...
    assert ( price_percent_change - ((price - old_price) / old_price * 100) < 1 ).all(), 'Columns do not add up'
    assert new_gainers.shape[1] == 4, new_gainers.shape
    assert new_gainers.symbol.dtype == 'O', f'Expected object (string) dtype but got {new_gainers.symbol.dtype}'
    assert new_gainers.price.dtype == new_gainers.price_change.dtype == new_gainers.price_percent_change.dtype == 'float', 'Expected numeric columns'
...
if __name__ == '__main__':
    ...
    filename = sys.argv[1]
    new_filename = filename.replace('.csv', '_norm.csv')
    read_gainers(filename).to_csv(new_filename)
